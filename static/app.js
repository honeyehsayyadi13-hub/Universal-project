...
function renderRouteBar() {
  ...
  rows.forEach(({ start, end }) => {
    ...
    for (let i = start; i < end; i++) {
      ...
      // Show the time you JOIN THE QUEUE, not the time you'd actually get
      // on the ride after waiting -- this matches the "queue by close"
      // rule, so the displayed time never exceeds closing for a stop
      // that fit, even though actual boarding happens after the wait.
